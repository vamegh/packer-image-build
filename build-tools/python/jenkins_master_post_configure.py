...
def main():
    ...
    secret_data = aws_secrets.get_secret(secret_names=config['aws_secrets'],
                                                region_name=config['aws']['aws_region'])
    oauth_creds = credentials_data['credentials_github_oauth_autobot']
    jenkins = jenkins_creds.Jenkins(config=config['jenkins'])
    jenkins.init_auth()
    for credentials in credentials_data:
        logging.info("Submitting Credentials: %s", credentials)
        cred_data = jenkins.generate_creds(data=credentials_data[credentials])
        jenkins.post(query_url='credentials/store/system/domain/_/createCredentials', payload=cred_data)

    for script in config['groovy_scripts']:
        logging.info("Executing Groovy Script: %s", script)
        script_data = jenkins.read_groovy(script=script)
        jenkins.post(query_url='scriptText', payload=script_data)

    for key, value in config['jenkins_master_settings'].items():
        value['script'] = os.path.join(config['temp_path'], value['script'])
        output_file = ("/tmp/groovy/%s.json" %(key))
        config_data = value['config']
        if key == 'github_oauth':
            config_data['client_id'] = oauth_creds['user']
            config_data['client_secret'] = oauth_creds['password']
        if key == 'jenkins_general':
            if isinstance(secret_data, dict):
                try:
                    config_data.update(secret_data['jenkins_master'])
                except Exception as err:
                    logging.warning("Unable to update config data from secrets manager")
                    logging.warning("Error as follows: %s", err)
        logging.info("Executing Groovy Script: %s :: Config Data: %s", value['script'], value['config'])
        handler.write_json(output_file=output_file, data=config_data)
        script_data = jenkins.read_groovy(script=value['script'], config_file = output_file)
        jenkins.post(query_url='scriptText', payload=script_data)
